/*
 * Author: a01sa01to
 * Submission URL: https://codeforces.com/contest/1660/submission/151554913
 * Submitted at: 2022-04-01 00:29:50
...
void solve() {
  ...
  if (zero.size() == 0) {
    ...
    if (zero[0] > n - zero.back() - 1) {
      if (minus[zero[0] + 1] % 2 == 0) {
        printf("%d %d\n", 0, n - zero[0]);
      }
      else {
        pair<int, int> l_coho(0, 0), r_coho(0, 0);
        ...
        if (l_coho < r_coho) {
          ...
          printf("%d %d\n", 0, n - (zero[0] + r_coho.second));
        }
      }
    }
    else {
      if ((minus[n] - minus[zero.back()]) % 2 == 0) {
        printf("%d %d\n", zero.back() + 1, 0);
      }
      else {
        pair<int, int> l_coho(0, 0), r_coho(0, 0);
